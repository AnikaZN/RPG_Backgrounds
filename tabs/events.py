Core = [
"You suffered a tragedy: ",
"You gained a bit of good fortune: ",
"You fell in love, got married, or had a child.",
"You made an enemy of an adventurer. You may or may not be to blame.",
"You made a friend of an adventurer.",
"You spent time working in a job related to your background. Start the game with +2d6 GP.",
"You met someone important.",
"You went on an adventure: ",
"You had a supernatural experience: ",
"You fought in a battle: ",
"You committed a crime or were wrongly accused of doing so: ",
"You encountered something magical: ",
"Something truly strange happened to you: "
]

Adventures = [
"You nearly died. You have nasty scars on your body, and you are missing an ear, finger(s), or toe(s).",
"You suffered a grievous injury. Although the wound healed, it still pains you from time to time.",
"You were wounded, but in time you fully recovered.",
"You contracted a disease while exploring a filthy warren. You recovered, but you have a lasting mark.",
"You were poisoned by a trap or a monster. You recovered, but saving throws against poison are with disadvantage.",
"You lost something of sentimental value to you during your adventure.",
"You were terribly frightened by something you encountered and ran away, abandoning your companions to their fate.",
"You learned a great deal on your adventure. Pick one skill with which you will have advantage when you roll.",
"You found some treasure on your adventure. Gain 2d6 GP.",
"You found a considerable amount of treasure. Gain 1d20 + 50 GP."
]

Arcane = [
"You were charmed or frightened by a spell.",
"You were injured or scarred by the effect of a spell.",
"You witnessed a powerful spell being cast.",
"You drank a potion.",
"You found a spell scroll and succeeded in casting it.",
"You were affected by teleportation magic.",
"You turned invisible for a time.",
"You identified an illusion for what it was.",
"You saw a creature being conjured by magic.",
"Your fortune was read by a diviner."
]

Boons = [
"A friendly wizard gave you a spell scroll containing one cantrip.",
"You saved the life of a commoner who now owes you a life debt. They accompany you on your travels and performs mundane tasks for you, but will leave if neglected, abused, or imperiled.",
"You found a riding horse.",
"You found some money. Gain 1d20 GP.",
"A relative bequeathed you a simple weapon.",
"You found something interesting - gain one additional trinket.",
"The next time you visit a temple that you once provided a service for, you can be healed up to you HP maximum.",
"A friendly alchemist gifted you with a potion of healing or a flask of acid.",
"You found a treasure map.",
"A distant relative left you a stipend that enables you to live comfortably for 1d20 years."
]

Crime = [
'Murder',
'Theft',
'Burglary',
'Assault',
'Smuggling',
'Kidnapping',
'Extortion',
'Counterfeiting']

Punishment = [
"You did not commit the crime and were exonerated.",
"You committed the crime, but the authorities found you not guilty.",
"You were nearly caught in the act. You ad to flee and are now wanted.",
"You were caught and convicted. You served time for 1d4 years."
]

Supernatural = [
"You were captured and enslaved by a fey for 1d6 years.",
"You saw a demon, but ran away before it could do anything to you.",
"A devil tempted you. Shift your alignment one more step towards evil or chaotic and gain 1d20 + 50 GP.",
"You woke up one morning miles from your home, with no idea how you got there.",
"You visited a holy site and felt the prescence of the divine there.",
"You witnessed a falling red star. You are certain it was an omen of some kind.",
"You escaped certain death and believe it was the intervention of a god that saved you.",
"You witnessed a minor miracle.",
"You explored an empty house and found it to be haunted.",
"You were briefly possessed.",
"You saw a ghost.",
"You saw a ghoul feeding on a corpse.",
"A celestial or a fiend visited you in a dream to warn you of dangers to come.",
"You briefly visited the Feywild or Shadowfell.",
"You saw a portal that you believe leads to another plane."
]

Tragedies = [
"A terrible blight in your home community caused crops to fall, and many starved. You lost a family member.",
"A family member or close friend died.",
"A friendship ended bitterly, and the other person is now hostile towards you.",
"You lost all your possessions in a disaster and had to rebuild your life.",
"You were imprisoned for a crime you didn't commit and served 1d6 years.",
"War ravaged your home community, reducing everything to rubble and ruin.",
"A lover disappeared without a trace. You have been looking for them since.",
"You did something that brought terrible shame to you in the eyes of your family.",
"For a reason you were never told, you were exiled from your community.",
"A romantic relationship ended.",
"A current or prospective romantic partner died."
]

War = [
"You were knocked out and left for dead. You woke up with no recollection of the battle.",
"You were badly injured in the fight, and still bear the scars.",
"You ran away from the battle to save your life, but still feel shame for your cowardice.",
"You suffered only minor injuries.",
"You survived the battle, but suffer from terrible nightmares in which you relive it.",
"You escaped the battle unscathed, but lost many friends.",
"You acquitted yourself well in battle and are remembered as a hero."
]

Weird = [
"You were turned into a toad and remained in that form for 1d4 weeks.",
"You were petrified and remained a stone statue for some time.",
"You were enslaved by a hag, satyr, or other being and lived that way for 1d6 years.",
"A dragon held you as prisoner for 1d4 months until adventurers killed it.",
"You were taken captive by drow and lived as a slave in the Underdark.",
"You served a powerful adventurer as a hireling.",
"You went insane for 1d6 years and only recently regained your sanity.",
"A lover of yours was secretly a silver dragon.",
"You were captured by a cult and nearly sacrificed to the being they follow before you escaped.",
"You met a demigod, archdevil, archfey, demon lord, or titan and lived to tell the tale.",
"You were swallowed by a giant fish and spent a month in its stomach.",
"A powerful being granted you a wish, but you squandered it on something frivolous."
]
